...
#define RADIO_BUTTOM_WINDOW_RADIO_RADIUS 6
  ...
  if (cell_index->row == 0) {
    ...
    if (selected_player && selected_team) {
    	...
    	if (js_running && latitude && longitude) {
    		send_FA_LOCATION(true);
    		if (guide_id && strcmp(game_id, "0")) {
    			hints_log = set_new();
...
static void inbox_received_handler(DictionaryIterator *iterator, void *context)
{
	...
	Tuple *recv_tuple;
	int i = inbox_received_indentify(DictionaryIterator *iterator, recv_tuple);
...
static message_t *set_message(char *info)
{
	if (curr_msg) {
...
static void handle_message()
{
	switch (curr_msg->opCode) {
		case "GAME_STATUS":
			int i = curr_msg->numKrags - curr_msg->numClaimed;
...
static void set_location(char *info)
{
	...
	longitude = calloc(sizeof(info));
	if (latitude == NULL || longitude == NULL) {
		APP_LOG(APP_LOG_LEVEL_ERROR, "Error: malloc fail");
		return;
	}

	strcpy(latitude, pebble_strtok(info, "|"));
	strcpy(longitude, pebble_strtok(NULL, "|"));
}
...
static int inbox_received_indentify(DictionaryIterator *iterator, Tuple *recv_tuple)
{
...
static void send_FA_CLAIM(char *krag_id)
{
	if (game_id && pebble_id && selected_team && selected_player && latitude && longitude && krag_id) {
		...
		snprintf(temp, "opCode=FA_CLAIM|gameId=%s|team=%s|player=%s|latitude=%s|longitude=%s|kragId=%s",
			gameId, selected_team, selected_player, latitude, longitude, krag_id);
		send_string(temp);
	}
}
...
staic void tick_handler(struct tm *tick_time, TimeUnits units_changed)
{
